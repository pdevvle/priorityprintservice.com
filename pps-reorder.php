...
/* ============================================================================
 * Reorder page bootstrap
 * ----------------------------------------------------------------------------
 * Ensures the Reorder page contains the [pps_order_lookup] shortcode.
 * Originally added manually to staging DB; codified here so it survives DB
 * syncs and propagates to production via the normal plugin deployment path.
 *
 * Strategy:
 *  - Run lazily on admin_init, gated by a 1-hour transient to avoid overhead
 *  - Locate the page by slug 'reorders' (slug is stable across environments,
 *    unlike post IDs)
 *  - If the shortcode is already in the page content, do nothing
 *  - Otherwise insert a paragraph block containing [pps_order_lookup]
 *    immediately after the "Reorder your Project" heading, or prepend it
 *    if the heading can't be located (fallback)
 *  - Status is recorded in wp_option 'pps_reorder_shortcode_status' for
 *    visibility / debugging
 *
 * Safe to run repeatedly. The has_shortcode() check is the idempotency guard.
 * ========================================================================= */
if ( ! function_exists( 'pps_reorder_ensure_lookup_shortcode' ) ) :
add_action( 'admin_init', 'pps_reorder_ensure_lookup_shortcode' );
function pps_reorder_ensure_lookup_shortcode() {
    // Throttle: only check once per hour to avoid running on every admin request
    if ( get_transient( 'pps_reorder_shortcode_checked' ) ) {
        return;
    }
    set_transient( 'pps_reorder_shortcode_checked', 1, HOUR_IN_SECONDS );

    $page = get_page_by_path( 'reorders', OBJECT, 'page' );
    if ( ! $page ) {
        update_option(
            'pps_reorder_shortcode_status',
            'page_not_found at ' . current_time( 'mysql', true ),
            false
        );
        return;
    }
    ...
    // Idempotent guard
    if ( has_shortcode( $page->post_content, 'pps_order_lookup' ) ) {
        return;
    }

    $content = $page->post_content;
    $shortcode_block = "\n\n<!-- wp:paragraph -->\n<p>[pps_order_lookup]</p>\n<!-- /wp:paragraph -->\n";

    // Try to insert immediately after the "Reorder your Project" heading
    $heading_pattern = '/(<!-- wp:heading[^>]*-->[\s\S]*?Reorder your Project[\s\S]*?<!-- \/wp:heading -->)/';
    if ( preg_match( $heading_pattern, $content, $matches ) ) {
        $new_content = str_replace(
            $matches[1],
            $matches[1] . $shortcode_block,
            $content
        );
        $method = 'after_heading';
    } else {
        // Fallback: prepend at start of content
        $new_content = ltrim( $shortcode_block ) . "\n" . $content;
        $method = 'prepended';
    }
    ...
    if ( is_wp_error( $result ) ) {
        update_option(
            'pps_reorder_shortcode_status',
            'error (' . $result->get_error_message() . ') at ' . current_time( 'mysql', true ),
            false
        );
    } else {
        update_option(
            'pps_reorder_shortcode_status',
            'installed_' . $method . ' at ' . current_time( 'mysql', true ),
            false
        );
    }
}
