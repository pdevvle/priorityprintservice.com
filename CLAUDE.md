...
- **Perfect bound gets the saddle/flat layout controls, per half (1.30)**: manual columns × rows, per-gap gutters, full-sheet fill and a press-sheet pick — **twice**, because the cover and the book block are different pieces on different stock and cannot share a column count or a gap array. `spec.pbGrid = {cover:{…}, guts:{…}}`, whitelisted through `pbPartOverrides()` (`gridAcross/gridDown/gridSheet/colGaps/rowGaps/fillSheet/gutter`) so a per-half override can never reach trim size, spine or artwork handling. The **top-level** grid/gutter keys are now cleared for both halves rather than applied to both — sharing them was incoherent (one gap array sized for two different grids) and produced a spurious "gutters were set for a different grid" warning on whichever half lost. The UI is one panel with a Cover/Guts selector; both halves' layouts are computed from the spec alone, so block-vs-sheet feedback is live before any file is dropped. The manual-grid warning no longer claims "pricing assumed 0-up" on an unpriced job. Also exposes the flats' `gridSheet` press-sheet select, which had state but no control.
- **Perfect bound inherited the saddle orientation trap, fixed (1.30)**: PB binds on the height edge, so the two size boxes are width × height exactly as for saddle — but the drop-time autodetect and the trim re-apply both tested `isSaddle` only, so dropping a 5.5×8.5 book normalised it to 8.5×5.5 and built a **17.25″-wide cover** with every page turned 90°, silently. Both paths now test `isSaddle || isPB`, the drop-time flip warning covers PB, and `imposePerfectBound` **refuses** artwork whose orientation contradicts the spec — the same guard saddle got in v1.21, which PB never received when it landed in 1.27.
- **The same fault, three more times, and the structural fix (1.31).** Asked whether the 1.30 bug could recur, an audit of every `calc === "saddle"` / `isSaddle` site found it already had: (1) **`specFromRow`** — the wp-admin queue path — recovered width × height from `sizeLabel` for saddle only, so every perfect-bound order came off the queue landscape (silent in 1.27–1.29, refused by the 1.30 guard, right only now); (2) **`parseManifest`** mapped any "Booklet Specifications" manifest to `saddle`, so dropping a perfect-bound job's manifest switched the product; (3) **the flats' multi-page select** rendered for PB and its state (default `repeat`) reached the guts through `spec.multiMode || "gang"` — **from wp-admin, every perfect-bound book block imposed pages 1+2 only, on every cell**, the warning buried in the notes; the harness never saw it because it omits the key and hit the fallback. Root cause in all of them: `isSaddle` was standing in for two *properties* — binds-on-the-height-edge and is-one-reading-order-document — that saddle was merely the only product with. Fix: `BOOK_CALCS = ["saddle","perfect-bound","coupon"]` with `isBoundCalc()` / `isBookCalc()` (two names, one list, so a site says which property it depends on), every such site rewritten to the property, guts hard-wired to `gang`, `multiMode`/`sides` never sent for books, manifests read a `Binding:` line (the PB/coupon calculators now write one; an old manifest with none keeps width × height and asks for the binding instead of guessing saddle), `specFromRow` exported for tests. **Rule: a branch keyed on a product name must name the property it depends on instead — a new bound product goes in `BOOK_CALCS` and nowhere else.** Also: the regression normaliser cannot see the slug's date (it is inside Flate-compressed streams), so a baseline from a different calendar day is a guaranteed false positive — regenerate the baseline same-day.
- **Print-safety audit (1.34)** — asked to find everything in the browser-PDF path that would print wrong or fail in Fiery, fixtures were built to provoke each suspicion and the engine measured against them. Found and fixed: **(1) pdf-lib embeds a page with BBox `[0 0 w h]` of the MediaBox** — a non-zero MediaBox origin (Acrobat crops, `[-9 -9 621 801]`) put the art off by the origin and clipped it (measured −0.257″/−2.38″); **(2) the CropBox was ignored**, so a page with slug area printed the slug (2.8 M px) and was scaled ×0.47 to the wrong size. Both go through `pageBox()` (CropBox ∩ MediaBox, real origin) and `embedPageBox()` (explicit bounding box to `embedPage`) now — every measurement site, `detectTrim`, the CLEAN copy (which keeps the true MediaBox origin and copies the CropBox relative), saddle, flats, the PB cover builder and the greyscale raster; **(3) encrypted files imposed as garbage silently** — pdf-lib parses with `ignoreEncryption` but never decrypts, so an owner-password file produced ZERO ink and nine content-stream syntax errors with no warning; `loadPdf()` refuses at load, by name, with the Acrobat steps, on every source path including the drop and the queue; **(4) the page-level transparency `/Group` was dropped** from the form XObject, so blend modes composited against the sheet — `carryGroups()` copies it after flush (verified `/Group` + `/Multiply` survive); **(5) marks, fold guides and slug were RGB black** → rich black on the RIP, four plates under a 0.5 pt cut mark — now `grayscale(0)`, K only; **(6) the slug never said the duplex flip edge** the Fiery job must match — now `DUPLEX SHORT-EDGE FLIP` / `SIMPLEX`. Added preflight: **non-embedded fonts** listed by name (standard 14 noted, everything else warned — Fiery substitutes and text re-wraps); **mixed page sizes** listed by page with what the tool does to them; **2-page spread detection** (pages ≈ 2× trim width) refusing bound products with a plain message, plus an opt-in **Split 2-page spreads** pre-pass (`spec.splitSpreads`, reader order, halves via `embedPage` with half-width boxes — 8 spreads → 16 pages verified); and an **upload-size guard** — PHP now injects `maxUpload` (`wp_max_upload_size()`) and the tool refuses with the real reason instead of the "Unauthorized" a truncated POST produces, with `pps_impose_upload` naming `$_FILES` error codes. Regression suites unchanged from v1.29 apart from the deliberate K-only operators and slug token. Testing gotcha: pymupdf fixtures draw with a top-left origin; a hand-written PDF uses PDF coordinates.
- **Oversize pages crop at 100 % (1.48)** — a page larger than the finished size on both axes with no TrimBox/BleedBox (4.5×2.5 with crop marks for a 4×2 card) used to fall to "fit-to-trim scaled" (card printed at 80 % with the marks inside the cut). `resolveSideArt(…, oversize)` now centres a trim rect of exactly the finished size on the page at scale 1, so the bleed clip takes the ring and the marks are cut away; `spec.oversize` = `crop` (default) | `scale` (old behaviour) via "Artwork larger than the finished size" in General settings, in setups. Smaller pages still scale. Harness: `cases_oversize.json`, `fx_marks45.pdf`.
- **"Invalid header in flate stream: -1, -1" (1.47)** — pdf-lib's decoder (not pdf.js) on a `/FlateDecode` content stream with ZERO bytes, which some exporters write for a blank page; it only surfaces when the page is embedded. `repairPageContents()` inside `loadPdf()` (every source path) drops such streams and notes the page (`doc.__ppsRepairNote` → warnings) so a back that should carry art is never printed blank quietly; a NON-empty undecodable stream refuses by file and page with the decoder's reason. Harness: `cases_flate.json` (`fx_emptyflate.pdf`, `fx_badflate.pdf`, hand-written PDFs).
- **Default imageable margin is 0 (1.46, owner's decision)** — `SHEET_MARGIN = 0`, stock sheets' usable area = the whole sheet. The margin box / setups / per-job override are unchanged; the priced imp functions keep their own 18.5″ constant (parity untouched). Marks and slug sit at the sheet edge by default; a press that cannot image its outer edge needs a typed margin.
- **Stale result under a refusal (1.45)** — found by mocking the wp-admin bridge (`ui_wpflow.mjs`): loading a 12×3 order refused its priced 8-up, but the run already started under the tool's default 11×8.5 spec (the order's size reaches the debounced spec 450 ms after the artwork) finished afterwards and put "Download IMPOSED_11x8.5_2up" + Send-to-Drive under the red refusal. The regenerate effect's early return now bumps `genSeq` (invalidating in-flight runs) and the effect never starts while `spec !== debSpec`. The priced refusal now lists the most that fits every way (`physicalFitOptions()`), and every sheet/piece phrase an operator reads is width × height as it lies ("13×19 sheet lying 19 wide × 13 tall", "artwork 3 wide × 12 tall (90°)"; `stockOf`/`sheetWH`/`sheetWords`/`pieceWords`) — the Sheet orientation and Artwork rotation selects too. **Rule: an early return from the regenerate effect must invalidate in-flight runs.**
- **Artwork rotation on the sheet (1.44)** — owner: "give me the ability to rotate not just the parent sheet but the artwork itself on the sheet. I need one column of 5 rows." `spec.artRotate` (auto|0|90|180|270, flats/stickers, in setups): `artRotateOf()` → engine `forceRotated` ("no"/"yes") + `artTurn`, which `buildCells()` adds to every cell's `patRot` on top of the piece pattern (backs follow as any pattern). Flats send `forceRotated:"auto"` so a stale tri-state can't force them; saddle/PB keep theirs. "Piece as placed" readout under the Layout head and on the manual-grid panel. **Manual-grid refusals now name what DOES fit** (`manualGridAlternatives()`: a×d and d×a × 0°/90° × landscape/portrait × current, 0.125″, 0″ margins) — "5 pieces DO fit as: (1) 5 × 1 at 90° on the 13×19 landscape; (2) 1 × 5 at 0° on the 13×19 PORTRAIT". Harness: `cases_artrot.json`, `ui_artrot.mjs`.
- **Font gate (1.43)** — a font only NAMED in the PDF (no `FontFile*` in its descriptor) is **refused**, not warned: `fontNotes(doc, what, spec, missingOut)` throws `FONTS NOT EMBEDDED in <artwork|back file|gang file "x">: <names>` (the back-file and gang-file `try` blocks re-throw it — they used to swallow everything). Standard 14 stay a note. Override is `spec.allowMissingFonts`, a tick in the refusal banner: warnings get `⚠ FONT OVERRIDE`, `res.fontsMissing` (every PB part too), filename `_FONTS` (composes with `_UNAPPROVED`), toolbar "FONT OVERRIDE", red card with Revoke. **Not in `SETUP_FIELDS`, reset on every `art`/`backArt`/`gangFiles` change, and bulk impose (`specFromRow`) never sends it** — a named-only font fails its queue row and files nothing. Harness: `cases_fontgate.json`, `ui_fonts.mjs`.
- **Parent sheet orientation (1.42)** — `spec.sheetPortrait`: the sheet stood on end (`orientSheet()` applied inside `SH()` in both layout functions and on the override; same `key`, label "↕ portrait", filename token `P`, slug `SHEET PORTRAIT`). Duplex: `pageFlipOf(spec, sheet)` maps the physical flip to page terms (a short-edge flip on a portrait page is a row mirror), so `mirrorCells` and the tumble rule follow unchanged and the slug keeps the physical edge — verified on crop-mark geometry (every front mark matches a back mark mirrored top↔bottom on portrait, left↔right on landscape, never the other axis). Manual-grid refusals point at the toggle. Layout settings: "Sheet orientation" beside the margin; in setups. Harness: `cases_port.json`, `ui_port.mjs`.
- **Progress bar (1.41)** — `imposePdf(…, onProgress)` sets module-level `PROG`; `progress(label, frac)` reports from the wrapper stages and the core loops (reading pages, placing each form/sheet, checking ink on each side) and **yields a frame on every report** (rAF, 40 ms fallback) so the bar actually paints under synchronous pdf-lib work. Monotonic within a run (PB runs the core twice); a failure's banner adds "Stopped while: <stage>". Found and fixed on the way: `SheetViewport` read `res.pageLabels` off a null `res` for one render when a run was refused after a success — the error boundary swallowed the tool. Harness: `ui_prog.mjs` (MutationObserver on the bar text).
- **Orientation per perfect-bound half (1.40)** — `forceRotated` is in `PB_PART_KEYS`; `pbGridSpec()` emits it from the half's `rot` (`default` inherits the job's Grid orientation), so a cover and a book block can lie differently. Select in the half's header beside its press sheet with a live "now 3 × 1, rotated 90°" readout; the job-level select is relabelled "(job default)". Verified: job-level across rotates both; a half's own choice wins. Harness: `cases_pbrot.json`, `ui_pbrot.mjs`.
- **Spread orientation for saddle (1.39)** — the flats' `spec.forceRotated` now reaches `computeSaddleLayout()` (`allowedRot` on every grid pick: manual copies, step and repeat, efficient, priced search, best-fit fallback) and a saddle-only **Spread orientation on the sheet** select (Auto / along the sheet's long edge / across, rotated 90°) sits in Layout settings with a live "Now: 2 × 2, spreads along" hint. A forced orientation that cannot hold the priced count refuses naming the orientation and "set Spread orientation back to Auto" first. `booklet.spreadAlongX` already derived from `grid.rotated`, so pagination, tumbling and fold guides follow without change. 4×4 booklet: Auto 4 × 1 rotated, along 2 × 2, across 4 × 1. Harness: `cases_rot.json`, `ui_rot.mjs`.
- **Page and sheet sequences (1.38)** — two Specialty boxes, one grammar (`5`, `3-8`, `5x2`, `3-8x2`, `blank`, `all`, `last`; `parseSeq()`): **Page sequence** (`spec.pageSeq`) rebuilds the source run before the core — after greyscale, so greyscale numbers refer to the file as dropped — for flats, saddle and both PB halves; **Sheet sequence** (`spec.sheetSeq`) repeats/reorders the imposed physical sheets after the ink guard and before slipsheets (whose numbers then refer to the result). A number past the end or a typo **refuses** with the count named — never a silent skip. Both are done **in place** by `resequenceDoc()` (a repeat is a new page leaf sharing the original's content and resources; originals removed) because copying pages into a new document drops `/OCProperties` and reprints hidden layers. Live readouts (`SeqNote`) under each box; `res.sheetsImposed` recorded before resequencing. Not saved in setups. Harness: `cases_seq.json`, `ui_seq.mjs`.
- **Saved setups (1.37)** — a picker + Save as… / Update / Delete row at the top of the controls column. A setup is every control-column setting except the job's own facts (name, order/item, qty, per-page bleed overrides, dropped files); applying one over a loaded order keeps the order's product/size/sides/spine. Stored in `wp_options['pps_impose_setups']` via `wp_ajax_pps_impose_setups` (`op=get|save`, shared, last writer wins, sanitised to scalars/small arrays) with a `localStorage` mirror; standalone uses `localStorage` only. `SETUP_FIELDS` in `App` is the single list — a new control that should be saveable goes there. Harness: `ui_setups.mjs`.
- **Sheet margin and gutters that stick (1.36)** — owner report: margin/gutter settings not sticking, preview not reflecting them. Found and fixed: the **imageable margin applied only to a custom sheet** (`marginOf()` + `withMargin()` now apply it to auto/preset sheets through `SH(key)` in both layout functions; same key/label, only usable area moves; empty box = default); **gap boxes coerced per keystroke** so React ate the decimal point (arrays hold typed text, `layoutOpts` coerces via `gapNum`); **custom gaps silently dropped when the grid changed shape** (UI now resizes the arrays to the layout on screen, per PB half too); the **Gutter select was dead once per-gap gutters existed** (replaced by "custom per gap · uniform" — one control at a time); **two press-sheet selects** (manual-grid one removed for flats; the single Press sheet + margin now head the Layout section with a live "prints W×H″" line); **PB half sheet ignored without a manual grid** (`pbPartOverrides` sets `sheet` from `gridSheet`, always); **PB viewport and grid panel disagreed on which half was on screen** (one selection — panel buttons, toolbar tabs, and tabs exist before artwork; report shows each half's live notes); offset select hidden in fill mode. **Fill full sheet respects the margin** (follow-up report): `boundRect()` is the printable rect in fill mode too — the block is centred inside the imageable margin, **0″ = the whole sheet**, and a block that would cross the margin is refused naming the margin and the 0″ hint (manual grid, gutters, saddle). Byte-identical engine output at the default margin (one PB fill case loses a slug that sat in the unimaged margin). UI harness: `ui_gut.mjs`.
- **Second pass, fool-proofing (1.35)** — `getPageBox()` reads **inherited** CropBox/TrimBox/BleedBox through `getInheritableAttribute` (a crop on the `/Pages` node was invisible; measured: 6×4 crop on 8.5×11 imposed as 8.5×11); a TrimBox lying outside the page is ignored with a note; **JPEG EXIF orientation** honoured (`jpegOrientation` + `uprightJpeg` via `createImageBitmap`, re-encoded upright only when tagged, so CMYK JPEGs stay CMYK — orientation 6 and 8 previously came out 180° apart); raster **effective DPI** warned under 200; **output ink guard** — every side of every part rendered at `GUARD_DPI` (24) after imposition, art area (middle 80 %) measured: all sides empty while the source has ink → refuse, some empty → listed; gang-combo file sizes checked; `/UserUnit` warned; slug reads `DUPLEX FLIP ON SHORT (13in) EDGE`; 401/403 from admin-ajax reported as an expired session; PB single-file < 4 pages refused. **Runtime libraries vendored** in `imposition-vendor/` (react, react-dom, babel, pdf.js + worker, pdf-lib — identical to the harness's copies); `pps_impose_app` rewrites each CDN tag to the plugin copy when the file is present at the pinned size, so the queue opens with unpkg/cdnjs down. The server directory is **self-provisioned** by `pps_impose_vendor_provision()` on first page load (the deploy tooling cannot create sub-directories): each file fetched from its pinned URL and kept only if its SHA-256 + byte count match `pps_impose_vendor_map()`; failures back off an hour and fall back to the CDN tag. Deploy stays two files; a library bump changes repo file, tool URL and map hash in one commit. Review catch: a duplicate `const INK_DPI` would have silently dropped bleed-ink detection to 24 DPI under Babel's const→var rewrite — renamed `GUARD_DPI`; one predicate per constant name.
- **Workspace layout (1.33, from the owner's wireframe)**: a sticky **accordion controls column** on the left (General · Layout · Product · Bleed · Specialty · Markings — `Acc` component; sections unmount when closed, all settings live in App state) and on the right the **job strip** (order queue, collapsible, in wp-admin; drop zones; approval/threat notices), a **viewport toolbar** (Fit 2 / − / + / 100% / Rulers, Cover/Guts part tabs for PB, parity badge + sheet summary, Download / Send-to-Drive for the part on screen) and the **sheets viewport** — every side of the active output part in one horizontal strip with inch rulers on both axes per sheet (`SheetViewport` / `SheetSide` / `RulerT` / `RulerL`), sides rasterised lazily by pdf.js as they scroll into view with correctly sized placeholders so the scrollbar is honest; before any artwork it shows the live `LayoutSchematic` at the same zoom. Warnings, side reports and layout figures moved into a collapsible **Job report** below. The old stacked `PdfPreview` is gone. Wireframe deviations, deliberate: rulers are per-sheet rather than one fixed pair (alignment is trivial and every sheet gets them); a sixth **Product** section holds the saddle/PB/multi-page/gang-combo controls the wireframe had no home for.
- **Specialty settings (1.33)** — two new engine features, applied by the `imposePdf` wrapper around `imposeCore` (so `imposePerfectBound`'s halves call `imposeCore`, or the passes run twice — found by test): **Force greyscale print on pages** (`spec.greyPages`, "1, 3-5" of the SOURCE artwork) rasterises those pages at 300 DPI via pdf.js and re-embeds each as a **DeviceGray** image filling the original MediaBox (`forceGreyPagesBytes`), so they separate to K only; boxes and `/Rotate` untouched, annots dropped, and the warning says plainly those pages are no longer vector — vector recolouring would mean rewriting every colour operator, colour space, shading and image the page reaches. **Insert slipsheet** (`spec.slip = {on, when: before|after, sheets: "1, 5", design: C|M|Y|K|custom, customBytes}`, `insertSlipsheets`): sheet numbers are the **physical sheets of the imposed output** (duplex: sheet 1 = pages 1–2), a slipsheet is one physical sheet (two pages on duplex, design on both faces), solids are a single `DeviceCMYK` fill, a custom upload is embedded and fitted (page 1 front, page 2 back if present); inserted highest-sheet-first so indices stay valid, page labels spliced to match, invalid sheet numbers reported not guessed; not counted in run figures. Verified: DeviceGray XObjects on exactly the listed pages, solid K/C/Y sheets at the right positions front and back, PB greyscale applied to the reading-order source (page 2 = inside front cover, so it lands in the cover half), engine byte-identical to v1.29 with both features off.
- **Perfect-bound guts get their own "Leaves onto the sheet" chooser (1.32)** — `spec.pbGutsOrder`: `gang` (default — consecutive leaves side by side, one sheet = one book's worth, qty = books), `repeat` (every cell the same leaf, one form per leaf, run divides by cells — step and repeat, as saddle's signatures), `cutstack` (leaf list runs down the piles). Lives in the Perfect bound card, shown whenever the guts are being produced. Mapped by `pbGutsMode()` onto the flat pipeline's collation modes (`gang`/`each`/`stack`) — the operator never sees those names, and the flats' `multiMode` still cannot reach the guts. 1.31 had removed the leaked select without giving PB a control of its own, so there was no way to step-and-repeat a book block at all; the guts-only card text even still pointed at the removed select.
