...
- **Step and repeat (saddle)** (`spec.sigRepeat`): the same thing stated as a count rather than a shape — "print each signature 3× per sheet"; the tool picks the arrangement (fewest rows first) and promotes to 13×27.5 or the forced parent when needed. Pagination into signatures is untouched. Flagged like the manual grid (badge/warning/slug `STEP AND REPEAT nx per sheet`, out of the parity preflight); mutually exclusive with the manual grid in the UI, and the manual grid wins if a caller sets both.
