...
- **Ganged signatures (default) vs step and repeat (saddle)**: multi-cell saddle sheets place **consecutive** signatures side by side — 9 sigs 2-up = 5 forms (1+2, 3+4, 5+6, 7+8, 9), one sheet through the press = one book's worth; a 16pp 4-up book is now one sheet front/back. `spec.sigRepeat` (the "step and repeat" toggle) switches to one signature filling every cell, one form per signature, run divided — given as a **count** ("3× per sheet"), the tool picks the arrangement and promotes to 13×27.5/the forced parent. Pagination into signatures is identical in both; only cell assignment changes. Uneven ganging leaves the last form's spare cells blank (filling them would print unequal quantities) and the warning quantifies the sheet cost; fewer sigs than cells tiles when the cells divide evenly (2 sigs in 4 cells → 1,2,1,2). Labels/slug read `FORM f/F SIGS 1+2`; a preflight guard counts every page across the forms and refuses on any drop/duplicate. Flagged like the manual grid, out of the parity preflight.
