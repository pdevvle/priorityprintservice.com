...
| `tools-proof-carryover-test.php` | **The approval must survive an edit — and must not survive the wrong one.** Edit mode and reorder restore artwork as `{type:'existing', path}`, so the calculator cannot re-post `pps_proof_hash`/`pps_artwork_files`/`pps_prepress_review` and they were dropped on every edit. That was silent: the imposition tool only checks a line carrying a 64-hex hash, and a line with none is `unbound` — it skips the comparison AND names its output exactly as it names a verified one. `pps_proof_carryover_from()` now carries the three keys when the artwork and every print-affecting spec field are unchanged, and drops them (with an `error_log` reason) when anything else moved — a carried hash asserting a sign-off the spec no longer matches is worse than the missing one this closes. The signature is **deny-by-default**, so a metadata key added in future blocks carry-over rather than being silently ignored. Plain PHP, no harness. |
