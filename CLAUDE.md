...
- Add-bleed synthesizer for no-bleed files (`spec.addBleed`: mirror = vector edge reflection past trim, scale = enlarge to cover bleed box); only fires when the page truly lacks bleed, respects spine/butt-cut clips, composes with patterns + duplex.
