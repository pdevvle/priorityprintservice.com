...
| `tools-media-prep.py` | Prep marketing photos for the WordPress media library: auto-rotate, convert to sRGB, cap the long edge, strip all metadata (phone photos carry GPS), minify, and rename to a WP-friendly slug. Output format mirrors input; alpha never goes to JPEG. Writes `manifest.json` with before/after sizes plus empty `title`/`alt` slots to fill per job. `--copyright` writes an ownership claim back *after* the strip, so GPS still goes and the rights claim stays (ASCII-folded for EXIF, which cannot carry `©`). `--no-auto-rotate` / `--rotate` override a lying EXIF orientation tag. Defaults: 2000 px, q82. See "Photo rollout to the media library" below. |
