...
- Add-bleed synthesizer for no-bleed files (`spec.addBleed`: mirror = vector edge reflection past trim, scale = enlarge to cover bleed box); only fires when the page truly lacks bleed, respects spine/butt-cut clips, composes with patterns + duplex. Smart edge detect (`spec.addBleedSmart`, default on) rasterizes each page and measures ink vs trim: shy art (≤3/16″ gap) gets pre-scaled so synthesis never doubles the white sliver; all-white-margin pages (design) skip synthesis entirely. Per-page mode override via `spec.addBleedPages` {1-based page: off|mirror|scale}.
