...
- **Perfect bound** (1.27) — images as **two press jobs**, because it is two:
  ...
  *width × height* exactly as saddle's are — and 5.5×8.5 is a different product
  ...
  | 9 | **"Fill full sheet" ignored the margin** (second report, same day): the fill bound was the bare sheet edge, so a typed 0.5″ margin did nothing once fill was ticked, and the slug could be placed in the unprintable margin. | `boundRect()` returned the full sheet in fill mode. | Fill mode now centres the block **inside the imageable margin** — the margin is the operator's statement of what to keep clear, and **0″ means the whole sheet** (how a guillotine layout says so explicitly). A block that would cross the margin is refused with the margin named and the 0″ hint, in the manual-grid, gutter and saddle paths. Sheet line, checkbox caption, panel text and the FILL badge all say "centred, m″ margin". Every fill case in the regression suites had a block inside the default margin, so their geometry is unchanged; the one PB fill case loses a slug that used to sit at 0.1″ from the sheet edge, in the margin the press does not image — now omitted with the usual note. |
