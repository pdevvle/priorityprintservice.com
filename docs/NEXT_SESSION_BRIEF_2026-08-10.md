...
| ~~New Relic~~ **RESOLVED — nothing to remove anywhere** | Per `DEPLOY_QA_FIXES_REPORT.md`: the "failing NR chunk" was the QA browser's **ad blocker** killing the request locally. Server agent was never running, repo greps clean, no page-side injector exists. Consequence for the freeze retest: run it in a **clean browser profile with the blocker off**, after the owner's Rocket change AND after the compiled calculators deploy (addendum 2) — with in-browser Babel gone, the calculator-side stalls should be fixed regardless of Rocket. |
